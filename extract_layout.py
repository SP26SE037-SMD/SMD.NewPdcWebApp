...
if start_idx != -1:
    for i in range(start_idx, len(lines)):
        if '<div className="mt-5 grid grid-cols-1 gap-6' in lines[i]:
...
if start_idx != -1 and end_idx != -1:
    with open('layout_block.tsx', 'w') as f:
        f.writelines(lines[start_idx:end_idx+1])
    print(f"Extracted lines {start_idx+1} to {end_idx+1} into layout_block.tsx")
else:
