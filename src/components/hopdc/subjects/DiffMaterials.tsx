                                    ...
                                    >
                                        ...
                                        <td className="px-4 py-3">
                                            <div className="flex items-center gap-3">
                                                <div className="w-8 h-8 rounded-lg flex items-center justify-center shrink-0" style={{ background: isAdded ? '#d1fae5' : isRemoved ? '#ffe4e6' : `${C.primary}12`, color: isAdded ? '#059669' : isRemoved ? '#e11d48' : C.primary }}>
                                                    <span className="material-symbols-outlined text-[18px]">
                                                        {material.materialType === 'PDF' ? 'picture_as_pdf' : material.materialType === 'VIDEO' ? 'smart_display' : 'description'}
                                                    </span>
                                                </div>
                                                <div className="min-w-0">
                                                    <h4 className={`text-sm font-extrabold truncate max-w-[200px] ${isRemoved ? 'line-through decoration-rose-300 text-rose-900' : isAdded ? 'text-emerald-900' : 'text-[#2d342b]'}`} style={{ fontFamily: 'Plus Jakarta Sans, sans-serif' }}>
                                                        {material.title}
                                                    </h4>
                                                </div>
                                            </div>
                                        ...
                                        <td className="px-4 py-3">
                                            <span className={`px-2 py-1 rounded-lg text-[9px] font-black uppercase tracking-wide border ${isAdded ? 'bg-emerald-100 text-emerald-700 border-emerald-200' : isRemoved ? 'bg-rose-100 text-rose-700 border-rose-200' : ''}`} style={(!isAdded && !isRemoved) ? { background: `${C.primary}08`, color: C.primary, borderColor: `${C.primary}20` } : {}}>
                ...
                return (
                    ...
                    >
                        <div className="flex items-start justify-between mb-4">
                            <div className="flex items-center gap-3">
                                <div className="w-10 h-10 rounded-xl flex items-center justify-center shrink-0 shadow-sm" style={{ background: isAdded ? '#d1fae5' : isRemoved ? '#ffe4e6' : `${C.primary}1a`, color: isAdded ? '#059669' : isRemoved ? '#e11d48' : C.primary }}>
                                    <span className="material-symbols-outlined text-[20px]">
                                        {material.materialType === 'PDF' ? 'picture_as_pdf' : material.materialType === 'VIDEO' ? 'smart_display' : 'description'}
                                    </span>
                                </div>
                                <div>
                                    <span className="text-[9px] font-extrabold uppercase tracking-widest" style={{ color: isAdded ? '#059669' : isRemoved ? '#e11d48' : C.primary }}>
                                        {material.materialType || 'Document'}
                                    </span>
                        ...
                        <h3 className={`text-[15px] font-extrabold mb-4 line-clamp-2 leading-snug ${isRemoved ? 'line-through decoration-rose-300 text-rose-900' : isAdded ? 'text-emerald-900' : 'text-[#2d342b]'}`} style={{ fontFamily: 'Plus Jakarta Sans, sans-serif' }}>
                            {material.title}
                        </h3>
                        
                        <div className="absolute top-4 right-4">
                            {isAdded && <span className="px-2 py-1 rounded-lg bg-emerald-100 text-emerald-700 text-[9px] font-black uppercase">Added</span>}
                            {isRemoved && <span className="px-2 py-1 rounded-lg bg-rose-100 text-rose-700 text-[9px] font-black uppercase">Removed</span>}
                        </div>
                    </div>
                );
            })}
