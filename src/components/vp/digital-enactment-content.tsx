...
import { Check, Clock, FileText, Loader2, BarChart3, ChevronRight } from "lucide-react";
...
export default function DigitalEnactmentContent() {
  ...
  const fetchRequests = async () => {
    setLoading(true);
    try {
      // Bỏ API cũ, gọi GET /api/requests
  ...
  const handleRequestClick = async (request: RequestItem) => {
    ...
    try {
      ...
      if (data?.data) {
        ...
        const curId = reqData.curriculum?.curriculumId || request.curriculum?.curriculumId;
        const majId = reqData.major?.majorId || request.major?.majorId;
        
        // Lưu vào localStorage
        if (reqData.requestId) localStorage.setItem("requestId", reqData.requestId);
                        ...
                        <div className="space-y-1">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs font-bold text-zinc-400 uppercase tracking-widest">
                              {item.major?.majorName || item.curriculum?.major?.majorName || "N/A"}
                            </span>
                          </div>
                          <h4 className="text-xl font-bold text-zinc-900 group-hover:text-primary transition-colors">
                            {item.title || item.curriculum?.curriculumName || "Untitled Request"}
                          </h4>
