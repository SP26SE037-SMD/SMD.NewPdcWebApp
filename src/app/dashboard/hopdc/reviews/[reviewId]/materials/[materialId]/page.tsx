...
export default function HoPDCReviewMaterialBlocksPage({ params }: { params: Promise<{ reviewId: string, materialId: string }> }) {
    ...
    const { data: routeTaskData, isLoading: isTaskLoading } = useQuery({
        queryKey: ['hopdc-task-detail', taskId],
    ...
    const { data: materialRes, isLoading: isMaterialLoading } = useQuery({
        queryKey: ['hopdc-material-detail', materialId],
                ...
                <aside className="w-72 flex flex-col gap-y-4 p-6 h-full overflow-y-auto"
                    ...
                    <div className="mb-6">
                        ...
                        <div className="flex flex-col gap-3 mb-8">
                            <div className="flex items-center justify-between">
                                ...
                                <button
                                    ...
                                    className="flex items-center gap-2 px-3 py-1.5 text-white rounded-lg text-[10px] font-black uppercase tracking-widest transition-all active:scale-95 shadow-sm"
                                    style={{
                            ...
                            <div className="flex flex-col gap-1">
                                ...
                                <div className="flex items-center gap-2 mt-1">
                                    ...
                                    <span className="text-[10px] font-medium" style={{ color: 'rgba(45,52,43,0.6)' }}>HoPDC Review</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="flex items-center gap-3 mb-4 p-3 rounded-2xl bg-[#ebf0e5] border border-[#dee5d8]/50">
                            ...
                            <div>
                                <h3 className="text-sm font-bold" style={{ color: '#2d342b' }}>HoPDC Mode</h3>
                                    ...
                                    <nav className="space-y-1">
                                        {outlineItems.map(item => (
                                            <button key={item.id} onClick={() => scrollToBlock(item.id)} title={item.content.length > 80 ? item.content.substring(0, 80) + '...' : item.content}
                                                className="w-full text-left py-2 px-3 truncate transition-all rounded-lg relative group overflow-hidden"
                                                style={{
                            ...
                            <div className="p-4 rounded-2xl bg-white border border-[#dee5d8] shadow-sm">
                                ...
                                <div className="space-y-2">
                                    <div className="flex items-center justify-between text-xs">
                                        <span className="font-medium" style={{ color: '#5a6157' }}>Total Blocks</span>
                                    ...
                                    <div className="flex items-center justify-between text-xs">
                                        <span className="font-medium" style={{ color: '#5a6157' }}>Headings</span>
                                        <span className="font-black" style={{ color: '#2d342b' }}>{outlineItems.length}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
