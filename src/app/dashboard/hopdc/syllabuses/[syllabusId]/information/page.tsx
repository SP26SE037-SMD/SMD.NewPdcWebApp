...
export default function InformationPage({ params }: { params: Promise<{ syllabusId: string }> }) {
    const { syllabusId } = use(params);
    const dispatch = useDispatch<AppDispatch>();

    // Fetch Syllabus Data
    const { data: syllabusRes, isLoading: isSyllabusLoading } = useQuery({
        queryKey: ['hopdc-syllabus-info', syllabusId],
    ...
    const { data: subjectRes, isLoading: isSubjectLoading } = useQuery({
        queryKey: ['hopdc-subject-detail', subjectId],
    ...
    useEffect(() => {
        const fetchInfo = async () => {
            ...
            try {
                ...
                // Without task, sources might be limited to subject data if available.
                // Assuming we don't have sources array on syllabus object directly.
                let sourcesReference: string[] = ["No references available."];
                ...
                dispatch(setSyllabusInfo({
                    ...
                    info: {
                        ...
                        sourcesReference: sourcesReference,
    ...
    if ((!!syllabusId && isSyllabusLoading) || (!!subjectId && isSubjectLoading)) {
        return (
        ...
        return (
            <div className="flex flex-col items-center justify-center py-20 px-4 text-center">
                ...
                <p style={{ color: C.onSurfaceVariant }} className="max-w-sm">
                    We couldn't retrieve the subject or syllabus details.
                </p>
            </div>
        );
    }
