...
export default function HoPDCRequestsPage() {
  ...
  const handleFixCurriculum = async (request: RequestItem) => {
    ...
    try {
      ...
      if (status === "DRAFT") {
        // For HoPDC, we redirect to assignments or sprint management
        ...
        if (task) {
          ...
          toast.error("Related task not found.", {
            id: toastId,
          });
        }
      } else {
        // Default navigation to sprint management
  ...
  const handleContinueToSprint = (request: RequestItem) => {
    // Navigate to sprint management for HoPDC
          ...
          >
            <div>
              ...
              <p className="text-on-surface-variant mt-2 text-base max-w-xl">
                Monitor and manage all change requests submitted within your program development cycle.
              </p>
            </div>
