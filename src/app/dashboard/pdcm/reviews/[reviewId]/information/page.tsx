...
export default function PDCMReviewInformationPage({
  params,
}: {
  ...
  const subjectId = syllabusData?.subjectId || (syllabusData as any)?.subject?.subjectId;
  ...
  const syllabusInfoDB = useSelector(
    (state: RootState) => state.syllabus.syllabusInfoDB,
  );
  ...
  useEffect(() => {
    const fetchInfo = async () => {
      ...
      try {
        ...
        const subjectId = data?.subjectId || (data as any)?.subject?.subjectId;
        ...
        let closText: string[] = [];
        if (
          closResult.status === "fulfilled" &&
          (closResult.value as any)?.data?.content
        ) {
          const closData = (closResult.value as any).data.content;
          closText = closData.map(
            (c: any) => `[${c.cloCode}] ${c.description}`,
          );
        }
        ...
        dispatch(
          setSyllabusInfo({
            ...
            info: {
              bloomTaxonomy: "N/A",
              sourcesReference: ["Failed to load. Please try again."],
              clos: ["Failed to load. Please try again."],
            },
          }),
        );
      }
  ...
  const activeBloomNum =
    subjectData?.minBloomLevel ?? syllabusData?.minBloomLevel;
  const bloomLabel =
    activeBloomNum && BLOOM_LABELS[activeBloomNum]
      ? BLOOM_LABELS[activeBloomNum]
      : displayInfo.bloomTaxonomy || "Loading...";

  const bloomNum =
    activeBloomNum ??
    parseInt(displayInfo.bloomTaxonomy?.match(/\d+/)?.[0] || "0");
  ...
  if (!syllabusData) {
    return (
      <div className="flex flex-col items-center justify-center py-20 px-4 text-center">
        ...
        <h3 className="text-lg font-bold mb-2" style={{ color: C.onSurface }}>
          No Syllabus Information
        </h3>
        <p style={{ color: C.onSurfaceVariant }} className="max-w-sm">
          We couldn&apos;t retrieve the specific syllabus details for this
          review task.
        </p>
      </div>
    );
  ...
  return (
    <div className="space-y-7">
      ...
      <div className="grid grid-cols-12 gap-5">
        {/* Bloom's Taxonomy */}
        <section
          className="col-span-12 lg:col-span-4 rounded-xl p-5 flex flex-col justify-between"
          style={{
            background: `${C.primaryContainer}4d`,
          ...
          <div className="mt-5 flex gap-2">
            {[1, 2, 3, 4, 5, 6].map((lvl) => (
              <span
                ...
                style={{
                  background: lvl <= bloomNum ? C.primary : C.surfaceVariant,
                }}
              />
            ))}
          </div>
                ...
                {col.map((ref, idx) => {
                  ...
                  return (
                    <li key={idx} className="flex items-start gap-2.5">
                      <span
                        className="w-5 h-5 flex items-center justify-center rounded-full text-[10px] font-bold shrink-0 mt-0.5"
                        style={{
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          },
          {
            ...
            value: subjectData?.decisionNo
              ? `Decision: ${subjectData.decisionNo}`
              : `Level ${syllabusData.decisionLevel || 1}`,
            highlight: true,
          },
        ].map((item) => (
          <div
            key={item.label}
            className="rounded-xl p-4 flex items-center gap-3 border-2 border-primary-500"
            style={{
              background: item.highlight
                ? `${C.primaryContainer}4d`
                : C.surfaceContainerLowest,
              border: item.highlight
                ? `1px solid ${C.primary}33`
                : `1px solid ${C.outlineVariant}44`,
            }}
          >
            <div
      ...
      >
        <div
          ...
          style={{ borderBottom: `1px solid ${C.outlineVariant}1a` }}
        >
            ...
            return (
              ...
              >
                <div className="shrink-0 mt-0.5">
                  <span
                    className="inline-flex items-center justify-center px-1.5 py-0.5 rounded-md text-[9px] font-bold tracking-tight uppercase"
                    style={{
                ...
                <div className="flex-1">
                  ...
                  <p
                    className="mt-0.5 max-w-4xl leading-relaxed text-[11.5px]"
                    style={{ color: C.onSurfaceVariant }}
                  >
                    {description}
                  </p>
