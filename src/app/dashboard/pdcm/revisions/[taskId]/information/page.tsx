...
export default function RevisionInformationPage({ params }: { params: Promise<{ taskId: string }> }) {
        ...
        return (
            <div className="flex flex-col items-center justify-center py-20 px-4 text-center">
                ...
                <p style={{ color: C.onSurfaceVariant }} className="max-w-sm">
                    We couldn't retrieve the specific syllabus details for this revision task.
                </p>
            </div>
        );
    }
                ...
                {[
                    ...
                    <div key={item.label}
                        ...
                        <div className="w-9 h-9 rounded-lg flex items-center justify-center shrink-0"
                            ...
                            <span className="material-symbols-outlined text-xl" style={{ color: C.primary }}>{item.icon}</span>
                        </div>
                        <div>
                            <p className="text-[9px] font-bold uppercase tracking-widest mb-0.5" style={{ color: C.onSurfaceVariant }}>{item.label}</p>
                            <p className="text-sm font-extrabold" style={{ color: item.highlight ? C.primary : C.onSurface }}>{item.value}</p>
                        </div>
                    </div>
                ))}
            </div>
            ...
            <section className="rounded-xl overflow-hidden shadow-sm" style={{ background: C.surfaceContainerLow }}>
                <div className="px-6 py-4 flex items-center justify-between" style={{ borderBottom: `1px solid ${C.outlineVariant}1a` }}>
                    <div className="flex items-center gap-3">
                        ...
                        <h3 className="text-lg font-bold" style={{ color: C.onSurface, fontFamily: 'Plus Jakarta Sans, sans-serif' }}>Course Learning Outcomes (CLOs)</h3>
                    </div>
                    <span className="text-[10px] uppercase tracking-widest font-bold" style={{ color: C.onSurfaceVariant }}>{displayInfo.clos.length} Outcomes defined</span>
                </div>
                            ...
                            >
                                <div className="shrink-0 mt-0.5">
                                    <span className="inline-flex items-center justify-center px-1.5 py-0.5 rounded-md text-[9px] font-bold tracking-tight uppercase"
                                        style={{ background: colorStyle.bg, color: colorStyle.text }}>{code}</span>
                                </div>
                                <div className="flex-1">
                                    <h4 className="font-bold text-sm" style={{ color: C.primary, fontFamily: 'Plus Jakarta Sans, sans-serif' }}>{code}</h4>
                                    <p className="mt-0.5 max-w-4xl leading-relaxed text-[11.5px]" style={{ color: C.onSurfaceVariant }}>{description}</p>
                                </div>
                            </div>
                        );
                    })}
