...
export default function RevisionWorkspaceLayout({
    children,
    params
}: {
    ...
    const displayTitle = realTask?.taskName || 'Revision Workspace';
    ...
    const sidebarItems = navItems.filter(n => !n.isAction).map(n => ({
        ...
        onClick: () => router.push(`/dashboard/pdcm/revisions/${displayId}/${n.id}`)
    }));
    ...
    const sidebarSubContent = (
        <div className="mt-4 p-4 rounded-2xl bg-primary/5 border border-primary/10">
            <p className="text-[10px] font-black text-primary uppercase tracking-widest mb-1">Revising Syllabus</p>
    ...
    return (
        <PDCMBaseLayout
            headerTitle="Revision Management"
            headerTabs={globalHeaderTabs}
            sidebarItems={sidebarItems}
            sidebarSubContent={sidebarSubContent}
            onBack={() => router.push('/dashboard/pdcm')}
            actionButton={{
                label: 'Resubmit Syllabus',
                icon: 'send',
                onClick: () => router.push(`/dashboard/pdcm/revisions/${displayId}/submit`)
            }}
        >
            ...
            <div className="flex flex-col gap-3 fixed bottom-10 right-10 z-50">
                {(realTask?.description || realTask?.comment) && (
                    ...
                    >
                        ...
                        <span className="font-bold text-xs whitespace-nowrap">Requirement</span>
                    </button>
                )}
