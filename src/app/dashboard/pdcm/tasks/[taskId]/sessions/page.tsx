                                                        ...
                                                        <React.Fragment key={idx}>
                                                            <tr className={`group transition-colors ${hasError ? 'bg-red-50/70 hover:bg-red-100/70' : hasWarning ? 'bg-amber-50/70 hover:bg-amber-100/70' : 'hover:bg-primary/5'}`}>
                                                                <td className="px-4 py-3 font-medium text-slate-700 text-center relative">
                                                                    ...
                                                                    {(hasError || hasWarning) && (
                                                                        <div className="absolute left-full top-1/2 -translate-y-1/2 ml-3 hidden group-hover:flex flex-col z-[60] min-w-[300px] max-w-[450px] bg-white/95 backdrop-blur-xl border shadow-2xl rounded-2xl overflow-hidden pointer-events-none animate-in fade-in zoom-in-95 slide-in-from-left-2 duration-200" style={{ borderColor: hasError ? 'rgba(239, 68, 68, 0.2)' : 'rgba(245, 158, 11, 0.2)' }}>
                                                                            {/* Caret pointing left */}
                                                                            <div className="absolute top-1/2 -left-1.5 -translate-y-1/2 w-3 h-3 bg-white border-l border-b rotate-45 shadow-sm" style={{ borderColor: hasError ? 'rgba(239, 68, 68, 0.2)' : 'rgba(245, 158, 11, 0.2)' }}></div>
                                                                            
                                                                            {/* Header */}
                                                                            <div className={`px-4 py-2.5 text-[10px] font-black uppercase tracking-widest border-b relative z-10 ${hasError ? 'bg-red-50/80 text-red-600 border-red-100/50' : 'bg-amber-50/80 text-amber-600 border-amber-100/50'}`}>
                                                                                <div className="flex items-center gap-1.5">
                                                                                    <span className="material-symbols-outlined text-[16px]">{hasError ? 'error' : 'warning'}</span>
                                                                                    {hasError ? 'Validation Errors' : 'Validation Warnings'}
                                                                                </div>
                                                                            </div>
                                                                            ...
                                                                            <div className="p-3 flex flex-col gap-2 relative z-10">
                                                                                {item._importErrors?.map((err: string, i: number) => (
                                                                                     <div key={`err-${i}`} className="flex gap-2.5 text-red-700 items-start text-[11px] font-medium leading-relaxed bg-red-50/50 p-2.5 rounded-xl border border-red-100/50">
                                                                                         <span className="material-symbols-outlined text-[16px] shrink-0 mt-0.5 text-red-500">cancel</span>
                                                                                         <span style={{ whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>{err}</span>
                                                                                     </div>
                                                                                 ))}
                                                                                 {item._importWarnings?.map((warn: string, i: number) => (
                                                                                     <div key={`warn-${i}`} className="flex gap-2.5 text-amber-700 items-start text-[11px] font-medium leading-relaxed bg-amber-50/50 p-2.5 rounded-xl border border-amber-100/50">
                                                                                         <span className="material-symbols-outlined text-[16px] shrink-0 mt-0.5 text-amber-500">warning</span>
