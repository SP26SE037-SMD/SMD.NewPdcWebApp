  ...
  try {
    ...
    if (!oldSyllabusId || !newSyllabusId) {
      return NextResponse.json(
        { error: "Missing syllabus IDs for copying" },
        { status: 400 },
      );
    }
    ...
    const backendResponse = await fetch(
      `${BACKEND_URL}/api/syllabus/copy?oldSyllabusId=${oldSyllabusId}&newSyllabusId=${newSyllabusId}`,
      {
        method: "POST",
        headers: {
          accept: "*/*",
    ...
    console.error("[API /syllabus/copy POST] Error:", error);
    return NextResponse.json(
