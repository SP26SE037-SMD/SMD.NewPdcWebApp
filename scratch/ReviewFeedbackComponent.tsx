...
import { Sparkles, TrendingDown, TrendingUp, Minus, AlertTriangle, Link2, BookOpen, ChevronUp, ChevronDown } from 'lucide-react';
...
export function ReviewerFeedback({ feedbackJson }: { feedbackJson: string }) {
    ...
    return (
        <div className="bg-white border-2 border-rose-100 rounded-[32px] overflow-hidden shadow-lg shadow-rose-100/50 mb-8">
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="grid gap-2">
                                {section.unmappedClos.map((c: any, i: number) => (
                                    <div key={i} className="flex gap-3 bg-rose-50/50 rounded-xl p-3 border border-rose-100">
                                        <span className="text-[10px] font-black text-rose-500 bg-rose-100 px-2 py-1 rounded-lg h-fit">{c.code}</span>
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="grid gap-2">
                                {section.unmappedAssessments.map((a: any, i: number) => (
                                    <div key={i} className="flex gap-3 bg-rose-50/50 rounded-xl p-3 border border-rose-100">
                                        <span className="text-[10px] font-black text-rose-500 bg-rose-100 px-2 py-1 rounded-lg h-fit">{a.questionType}</span>
